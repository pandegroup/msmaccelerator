...
{
  // Begin Monakos modifications ~~~ (due to LPW)
  int optval;
  socklen_t optlen = sizeof(optval);
  /* Check the status for the keepalive option */
  if(getsockopt(link->fd, SOL_SOCKET, SO_KEEPALIVE, &optval, &optlen) < 0) {
    perror("getsockopt()");
    close(link->fd);
    exit(EXIT_FAILURE);
  }
  ...
  if(setsockopt(link->fd, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen) < 0) {
    perror("setsockopt()");
    close(link->fd);
    exit(EXIT_FAILURE);
  }
  ...
  //printf("SO_KEEPALIVE is now %s\n", (optval ? "ON" : "OFF"));
  // End Monakos modifications ^^^
